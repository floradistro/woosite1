...
export default function ProductCarousel({ 
  ...
  const getIconStyling = () => {
    if (productType === 'vape') {
      ...
      return {
        container: "absolute bottom-2 -right-4 w-24 h-24 md:w-32 md:h-32 opacity-100 transition-all duration-300 transform group-hover/image:scale-110 shadow-lg flex items-center justify-center",
        image: "w-full h-full object-contain"
      };
    } else {
      // All other product types - with backdrop and border
      return {
        container: "absolute bottom-2 right-2 w-24 h-24 md:w-32 md:h-32 rounded-full overflow-hidden border-2 border-white/40 bg-black/30 backdrop-blur-sm opacity-80 group-hover/image:opacity-100 transition-all duration-300 transform group-hover/image:scale-110 shadow-lg flex items-center justify-center p-1",
        image: "w-full h-full object-contain"
      };
    }
  };
  ...
  if (products.length === 0) {
    return (
      <section className="py-8 md:py-16 bg-[#4a4a4a]">
  ...
  const ProductCard = ({ product }: { product: CarouselProduct }) => (
    <div className="bg-black/30 rounded-xl p-4 md:p-6 h-full backdrop-blur-sm border border-white/10 hover:border-white/20 transition-all duration-300 group">
      {/* Product Image */}
      <div className="aspect-square mb-3 md:mb-4 rounded-lg overflow-hidden bg-black/20 relative group/image">
        <img
          src={product.image}
          alt={product.title}
        ...
        {productType === 'moonwater' && (
          <div className="absolute top-0 left-0 right-0 flex flex-col justify-start items-center text-center px-2 md:px-3 pt-4 md:pt-8 pointer-events-none transform -rotate-3 transition-all duration-500 group-hover:scale-105 group-hover:-rotate-2">
            <h3 className="text-white font-bold text-2xl md:text-4xl lg:text-5xl mb-1 leading-none transform transition-all duration-700 animate-pulse group-hover:scale-110" 
                style={{ 
            ...
            <div className="text-white font-bold text-sm md:text-lg lg:text-xl tracking-wider transform transition-all duration-500 group-hover:tracking-widest group-hover:scale-105" 
                 style={{ 
      ...
      <div className="space-y-2 md:space-y-3">
        ...
        <div className="flex justify-between items-center pt-2">
          <div>
            ...
            <span className="text-white/50 text-xs md:text-sm ml-1">
              {getPriceUnit()}
            </span>
          </div>
        ...
        <button
          ...
          className="w-full bg-white/10 hover:bg-white/20 text-white py-2 px-4 rounded-lg transition-all duration-300 backdrop-blur-sm border border-white/20 hover:border-white/40 text-sm md:text-base"
        >
          Quick View
        </button>
      </div>
        ...
        <div className="max-w-7xl mx-auto px-2 md:px-4">
          ...
          <div className="text-center mb-8 md:mb-12 px-2 md:px-0">
            {title === "[shop moonwater]" ? (
              ...
              <h2 className="text-white font-bold text-3xl md:text-5xl lg:text-6xl leading-tight mb-4 md:mb-6 transform transition-all duration-700" 
                  style={{ 
                    fontFamily: 'Nunito Sans, Varela Round, sans-serif',
                    textShadow: '2px 2px 4px rgba(0,0,0,0.8)',
                    animation: 'fadeInUp 0.8s ease-out'
              ...
              // Edible hero styling
              <h2 className="text-white text-3xl md:text-5xl lg:text-6xl font-light tracking-wide transform hover:scale-105 transition-transform duration-200 uppercase mb-4 md:mb-6">
                {title}
              </h2>
            ) : (
              // Default hero styling for flower, vape, concentrate
              <h2 className="text-white text-3xl md:text-5xl lg:text-6xl font-light tracking-wide transform hover:scale-105 transition-transform duration-200 mb-4 md:mb-6">
                {title}
              </h2>
            )}
            {subtitle && (
          ...
          <div className="relative -mx-2 md:mx-0">
            {/* Navigation Arrows - Hidden on mobile for touch scrolling */}
            {products.length > itemsPerView && (
              <>
                <button
                  onClick={prevSlide}
                  className="hidden md:block absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-black/50 hover:bg-black/70 text-white p-2 rounded-full transition-all duration-300 backdrop-blur-sm"
                ...
                <button
                  onClick={nextSlide}
                  className="hidden md:block absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-black/50 hover:bg-black/70 text-white p-2 rounded-full transition-all duration-300 backdrop-blur-sm"
            ...
            {/* Dots Indicator - Smaller on mobile, hidden for mobile scroll */}
            {products.length > itemsPerView && (
              <div className="hidden md:flex justify-center mt-6 md:mt-8 gap-1 md:gap-2">
