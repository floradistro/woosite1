...
import { WEIGHT_PRICING, GRAM_PRICING, GRAM_SIZES, type FeaturedProduct, type ProductFormat } from '../constants';
...
export default function DenseView({
  ...
  const getDefaultSelection = (productId: number) => {
    return format === 'wax' ? '0.5g' : '1g';
  };

  const getCurrentOptions = () => {
    return format === 'wax' ? WEIGHT_PRICING : GRAM_PRICING;
  };

  const getAvailableSizes = () => {
    return format === 'wax' ? Object.keys(WEIGHT_PRICING) : GRAM_SIZES;
  };
                ...
                <div className={`${getCardStyling(product.id)} p-1.5 group-hover:shadow-xl relative ${
                  ...
                  <div className="flex items-start gap-1.5 mb-3 md:mb-4">
                    ...
                    >
                      ...
                      {/* Live resin indicator in top left of image */}
                      {format === 'live-resin' && (
                        <div className="absolute top-1 left-0 z-10 p-0">
                          <div className="bg-amber-500 text-white text-xs px-2 py-1 rounded-br-lg font-medium">
                            LIVE
                          </div>
                        </div>
                      )}
                      ...
                      <Image
                        src={product.image}
                        alt={format === 'live-resin' ? `${product.title} live resin` : product.title}
                        fill
                    ...
                    <div className="flex-1 min-w-0 relative">
                      {/* Title - without live resin indicator on mobile, with it on desktop */}
                      <h3 className={`font-extralight text-2xl md:text-xl transition-colors duration-300 ${
                        ...
                        <span className="hidden md:flex md:items-center md:gap-1">
                          ...
                          {format === 'live-resin' && (
                            <span className="text-amber-400 text-xs font-light whitespace-nowrap">live resin</span>
                          )}
                        </span>
                      </h3>
                      
                      {/* Live resin indicator - Mobile only, below title */}
                      {format === 'live-resin' && (
                        <p className="md:hidden text-amber-400 text-sm font-light mb-1">live resin</p>
                      )}
                      ...
                      <div className="flex flex-wrap gap-1 mb-1">
                        ...
                        {product.texture.map((textureType, idx) => (
                      ...
                      <div className="hidden md:block">
                        <p className={`text-xs leading-relaxed transition-colors duration-300 ${
                          ...
                          {format === 'live-resin' 
                            ? `Premium ${product.title} live resin concentrate. ${product.description}`
                            : product.description
                          }
                        </p>
                      </div>
