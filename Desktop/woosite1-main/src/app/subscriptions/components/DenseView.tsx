...
import { BOTTLE_PRICING, PACK_PRICING, BOTTLE_SIZES, PACK_SIZES, type FeaturedProduct, type ProductFormat } from '../constants';
...
export default function DenseView({
  ...
  const getDefaultSelection = (productId: number) => {
    return format === 'bottle' ? '1-bottle' : '4-pack';
  };

  const getCurrentOptions = () => {
    return format === 'bottle' ? BOTTLE_PRICING : PACK_PRICING;
  };

  const getAvailableSizes = () => {
    return format === 'bottle' ? Object.keys(BOTTLE_PRICING) : PACK_SIZES;
  };
                ...
                <div className={`${getCardStyling(product.id)} p-1.5 group-hover:shadow-xl relative ${
                  ...
                  <div className="flex items-start gap-1.5 mb-3 md:mb-4">
                    ...
                    >
                      ...
                      {/* Pack indicator in top left of image */}
                      {format === 'pack' && (
                        <div className="absolute top-1 left-0 z-10 p-0">
                          <div className="bg-cyan-500 text-white text-xs px-2 py-1 rounded-br-lg font-medium">
                            PACK
                          </div>
                        </div>
                      )}
                      ...
                      <Image
                        src={product.image}
                        alt={format === 'pack' ? `${product.title} pack` : product.title}
                        fill
                    ...
                    <div className="flex-1 min-w-0 relative">
                      {/* Title - without pack indicator on mobile, with it on desktop */}
                      <h3 className={`font-extralight text-2xl md:text-xl transition-colors duration-300 ${
                        ...
                        <span className="hidden md:flex md:items-center md:gap-1">
                          ...
                          {format === 'pack' && (
                            <span className="text-cyan-400 text-xs font-light whitespace-nowrap">pack</span>
                          )}
                        </span>
                      </h3>
                      
                      {/* Pack indicator - Mobile only, below title */}
                      {format === 'pack' && (
                        <p className="md:hidden text-cyan-400 text-sm font-light mb-1">pack</p>
                      )}
                      ...
                      <div className="flex flex-wrap gap-1 mb-1">
                        ...
                        {product.flavor.map((flavorNote: string, idx: number) => (
                      ...
                      <div className="hidden md:block">
                        <p className={`text-xs leading-relaxed transition-colors duration-300 ${
                          ...
                          {format === 'pack' 
                            ? `Multi-pack of ${product.title}. ${product.description}`
                            : product.description
                          }
                        </p>
                      </div>
