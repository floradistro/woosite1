  ...
  return (
    <>
      ...
      <section className="relative h-48 md:h-56 lg:h-64 overflow-hidden bg-[#4a4a4a] animate-fadeIn"
        style={{
          backgroundImage: `
            radial-gradient(circle at 20% 30%, rgba(255, 100, 150, 0.08) 0%, transparent 40%),
            radial-gradient(circle at 80% 70%, rgba(255, 140, 0, 0.06) 0%, transparent 35%),
            radial-gradient(circle at 60% 20%, rgba(255, 120, 100, 0.05) 0%, transparent 30%),
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center space-y-4 animate-fadeInUp px-6">
            <h1 className="text-white text-4xl md:text-5xl lg:text-6xl font-light tracking-wide transform hover:scale-105 transition-transform duration-200 uppercase">
              {format === 'single' ? 'EDIBLES' : 'BULK PACKS'}
            </h1>
            <h2 className="text-white/90 text-base md:text-lg lg:text-xl font-light tracking-wide max-w-2xl mx-auto drop-shadow-lg opacity-0 animate-fadeInUp" style={{ animationDelay: '0.5s', animationFillMode: 'forwards' }}>
              {format === 'single' 
                ? 'Precise dosing. Artisan quality. Delicious taste.'
                : 'Bulk savings. Consistent effects. Share & save.'
              }
            </h2>
            <div className="flex flex-col items-center gap-2 mt-4 opacity-0 animate-fadeInUp" style={{ animationDelay: '1s', animationFillMode: 'forwards' }}>
              <div className="flex items-center gap-2 text-sm">
                <span className="text-pink-400 font-medium drop-shadow-md hover:text-pink-300 transition-colors duration-200">✓ 5-25mg THC</span>
                <span className="text-white/40">•</span>
                <span className="text-pink-400 font-medium drop-shadow-md hover:text-pink-300 transition-colors duration-200">✓ Fast Acting</span>
                <span className="text-white/40">•</span>
                <span className="text-pink-400 font-medium drop-shadow-md hover:text-pink-300 transition-colors duration-200">✓ Natural Flavors</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Quality Banner */}
      <div className="bg-gradient-to-r from-pink-500 via-orange-500 to-pink-600 py-2 text-center z-40">
        <div className="flex flex-wrap items-center justify-center gap-4 md:gap-8 px-4">
          <span className="text-white font-medium text-sm md:text-base">✓ Lab Tested</span>
          <span className="text-white/40">•</span>
          <span className="text-white font-medium text-sm md:text-base">
            ✓ {format === 'single' ? 'Precise Dose' : 'Bulk Savings'}
          </span>
          <span className="text-white/40">•</span>
          <span className="text-white font-medium text-sm md:text-base">
            ✓ {format === 'single' ? 'Great Taste' : 'Share Ready'}
          </span>
        </div>
      </div>
    </>
