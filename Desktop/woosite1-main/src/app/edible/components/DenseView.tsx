...
import { SINGLE_PRICING, PACK_PRICING, PACK_SIZES, type FeaturedProduct, type ProductFormat } from '../constants';
...
export default function DenseView({
  ...
  const getDefaultSelection = (productId: number) => {
    return format === 'single' ? '1-piece' : '10-pack';
  };

  const getCurrentOptions = () => {
    return format === 'single' ? SINGLE_PRICING : PACK_PRICING;
  };

  const getAvailableSizes = () => {
    return format === 'single' ? Object.keys(SINGLE_PRICING) : PACK_SIZES;
  };
      ...
      <div className="w-full relative z-10">
        <div className="w-full md:columns-2 lg:columns-3 md:gap-1 space-y-2 md:space-y-0">
          {products.map((product, index) => {
            const selectedSize = selectedSizes[product.id] || getDefaultSelection(product.id);
            const currentOptions = getCurrentOptions();
            const price = currentOptions[selectedSize as keyof typeof currentOptions];
                ...
                <div className={`${getCardStyling(product.id)} p-1.5 group-hover:shadow-xl relative ${
                  ...
                  <div className="flex items-start gap-1.5 mb-3 md:mb-4">
                    ...
                    >
                      ...
                      {/* Bulk indicator in top left of image */}
                      {format === 'bulk' && (
                        <div className="absolute top-1 left-0 z-10 p-0">
                          <div className="bg-orange-500 text-white text-xs px-2 py-1 rounded-br-lg font-medium">
                            BULK
                          </div>
                        </div>
                      )}
                      ...
                      <Image
                        src={product.image}
                        alt={format === 'bulk' ? `${product.title} bulk pack` : product.title}
                        fill
                    ...
                    <div className="flex-1 min-w-0 relative">
                      {/* Title - without bulk indicator on mobile, with it on desktop */}
                      ...
                      {/* Bulk indicator - Mobile only, below title */}
                      {format === 'bulk' && (
                        <p className="md:hidden text-orange-400 text-sm font-light mb-1">bulk pack</p>
                      )}
                      ...
                      {/* THC */}
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-white/70 text-sm md:text-xs">THC:</span>
                        <span className="text-emerald-400 text-sm md:text-xs font-medium">{product.thc}mg</span>
                      </div>
                      ...
                      <div className="hidden md:block">
                        <p className={`text-xs leading-relaxed transition-colors duration-300 ${
                          ...
                          {format === 'bulk' 
                            ? `Bulk pack of ${product.title}. ${product.description}`
                            : product.description
                          }
                        </p>
                      </div>
                  ...
                  <div className="flex justify-end">
                    <div className="text-white/95 font-light text-lg md:text-xl group-hover:text-white transition-colors duration-300">
                      ${price} <span className="text-xs text-white/60">/{selectedSize}</span>
                    </div>
                  </div>
