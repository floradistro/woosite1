...
class ProductService {
  ...
  async getAllProducts(): Promise<WooProduct[]> {
    ...
    if (!USE_WOOCOMMERCE) {
      console.log('WooCommerce disabled, returning empty array');
      return [];
    }
    ...
    try {
      ...
      console.log(`Raw WooCommerce products fetched: ${wooProducts.length}`);
  ...
  async getProductsByType(productType: string): Promise<WooProduct[]> {
    console.log(`Getting products by type: ${productType}`);
    ...
    const filtered = allProducts.filter(product => {
      ...
      if (matches) {
        console.log(`Product "${product.title}" matches type "${productType}"`);
      }
      
      return matches;
    });
    
    console.log(`Found ${filtered.length} products of type "${productType}"`);
    return filtered;
  }
  ...
  async getProduct(id: number): Promise<WooProduct | null> {
    ...
    if (!USE_WOOCOMMERCE) {
      console.log('WooCommerce disabled, returning null');
      return null;
    }
